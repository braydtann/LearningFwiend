...
# =============================================================================
# FINAL EXAM MODELS AND ENDPOINTS
# =============================================================================

class FinalTestQuestion(BaseModel):
    id: str = Field(default_factory=lambda: str(uuid.uuid4()))
    type: str  # multiple-choice, select-all-that-apply, true-false, short-answer, long-form, chronological-order
    question: str
    options: Optional[List[str]] = None  # For multiple choice and select all
    correctAnswer: Optional[str] = None  # For multiple choice and true/false
    correctAnswers: Optional[List[str]] = None  # For select all that apply
    correctOrder: Optional[List[int]] = None  # For chronological order
    items: Optional[List[str]] = None  # For chronological order
    points: int = 1
    questionImage: Optional[str] = None
    questionAudio: Optional[str] = None
...
class FinalTestAttemptCreate(BaseModel):
    testId: str
    programId: Optional[str] = None
    answers: List[dict]  # [{"questionId": "...", "answer": "..."}]
...
@api_router.post("/final-tests", response_model=FinalTestResponse)
async def create_final_test(
    test_data: FinalTestCreate,
    current_user: UserResponse = Depends(get_current_user)
):
    ...
    return FinalTestResponse(**test_dict)
...
async def get_all_final_tests(
    ...
    """Get all final tests, optionally filtered by program."""
    query = {}
    if program_id:
        query["programId"] = program_id
    if published_only:
    ...
    tests = await db.final_tests.find(query).to_list(1000)
    return [FinalTestResponse(**test) for test in tests]

@api_router.get("/final-tests/{test_id}", response_model=FinalTestResponse)
async def get_final_test(
    test_id: str,
    current_user: UserResponse = Depends(get_current_user)
):
...
async def submit_final_test_attempt(
    ...
    """Submit a final test attempt."""
    # Only learners can submit attempts
    if current_user.role != 'learner':
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Only students can submit final test attempts"  
        )
    
    # Get the test to validate and calculate score
    test = await db.final_tests.find_one({"id": attempt_data.testId})
    if not test:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Final test not found"
    ...
    for question in test.get("questions", []):
        ...
        if question_type == "multiple-choice":
            if student_answer == question.get("correctAnswer"):
                correct_answers += 1
        elif question_type == "true-false":
            if str(student_answer).lower() == str(question.get("correctAnswer", "")).lower():
                correct_answers += 1
        elif question_type == "select-all-that-apply":
            correct_options = set(question.get("correctAnswers", []))
            student_options = set(student_answer if isinstance(student_answer, list) else [])
            if correct_options == student_options:
                correct_answers += 1
        elif question_type == "chronological-order":
            correct_order = question.get("correctOrder", [])
            if student_answer == correct_order:
                correct_answers += 1
        elif question_type in ["short-answer", "long-form"]:
            # For subjective questions, assume correct for now (needs manual grading)
            correct_answers += 1
    
    # Calculate percentage score
    score = (correct_answers / total_questions * 100) if total_questions > 0 else 0
    passed = score >= test.get("passingScore", 70)
...
async def get_final_test_attempts(
    ...
    """Get final test attempts (filtered by test or student)."""
    query = {}
    ...
    attempts = await db.final_test_attempts.find(query).to_list(1000)
    return [FinalTestAttemptResponse(**attempt) for attempt in attempts]
...
async def get_course_submissions(
    ...
    quiz_attempts = await db.quiz_attempts.find({"courseId": course_id}).to_list(1000)
    ...
    for attempt in quiz_attempts:
        ...
        for answer in attempt.get("answers", []):
            ...
            question_text = answer.get("questionText", "Unknown question")
            student_answer = answer.get("answer")
            
            # Check if this is a subjective question (short-answer or long-form)
            if answer.get("questionType") in ["short-answer", "long-form"]:
                submissions.append({
                    "id": f"quiz-{attempt['id']}-{question_id}",
                    "studentId": student_id,
...
async def grade_submission(
    ...
    # For now, store grading data in a separate collection
    # In a full implementation, this would update the original quiz/test attempt
    grading_dict = {
        "id": str(uuid.uuid4()),
