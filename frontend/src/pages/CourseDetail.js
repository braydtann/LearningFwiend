...
const CourseDetail = () => {
  ...
  const checkQuizRequirements = async () => {
    try {
      ...
      for (const quizLesson of quizLessons) {
        // Check passingScore in multiple possible locations
        const passingScore = quizLesson.passingScore || quizLesson.quiz?.passingScore;
        console.log(`🔍 DEBUG: Checking quiz "${quizLesson.title}" (ID: ${quizLesson.id})`);
        console.log(`   📊 Lesson level passingScore: ${quizLesson.passingScore}`); 
        console.log(`   📊 Quiz sub-object passingScore: ${quizLesson.quiz?.passingScore}`);
        console.log(`   📊 Final passingScore used: ${passingScore}`);
        ...
        if (!quizPassed && currentEnrollment?.moduleProgress) {
          for (const moduleProgress of currentEnrollment.moduleProgress) {
            const lessonProgress = moduleProgress.lessons?.find(l => l.lessonId === quizLesson.id);
            if (lessonProgress?.completed) {
              console.log(`✅ Quiz "${quizLesson.title}" marked as completed in moduleProgress - allowing completion`);
              quizPassed = true;
              break;
            }
          }
  ...
  const handleNextAction = async () => {
    ...
    try {
      ...
      if (nextAction.type === 'complete') {
        ...
        console.log(`🚀 DEBUG: Handling regular lesson/module navigation`);
        if (selectedLesson) {
  ...
  const canAccessQuiz = (quiz) => {
    ...
    // Find which module contains this quiz and its position globally
    let quizModuleIndex = -1;
    let quizLessonIndex = -1;
    let globalQuizIndex = -1; // Position among all quizzes in the course
    
    // First, collect all quizzes in order
    const allQuizzes = [];
    ...
    // Find current quiz in the list
    for (let k = 0; k < allQuizzes.length; k++) {
      if (allQuizzes[k].id === quiz.id) {
        quizModuleIndex = allQuizzes[k].moduleIndex;
        quizLessonIndex = allQuizzes[k].lessonIndex;
        globalQuizIndex = k;
        break;
      }
    }

    if (quizModuleIndex === -1 || quizLessonIndex === -1 || globalQuizIndex === -1) {
    ...
    // 2. For subsequent quizzes, check if previous quiz was completed successfully
    ...
    const prevQuizLessonProgress = prevQuizModuleProgress.lessons.find(lp => lp.lessonId === previousQuiz.id);
