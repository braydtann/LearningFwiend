      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <Card className="mb-6">
          ...
          <CardContent className="space-y-4">
              ...
              <div className="space-y-4">
                <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  <p className="text-orange-800 text-sm font-medium">
                    📝 Instructions: Drag and drop the items below to arrange them in the correct chronological order.
                  </p>
                </div>
                
                <div className="space-y-3">
                  {(Array.isArray(currentQuestion.items) ? currentQuestion.items : []).map((item, originalIndex) => {
                    // Handle both string and object item formats with defensive programming
                    const itemText = typeof item === 'string' ? item : (item?.text || `Item ${originalIndex + 1}`);
                    const itemImage = typeof item === 'object' ? item?.image : null;
                    const itemAudio = typeof item === 'object' ? item?.audio : null;
                    
                    // Get current answer (array of ordered indices)
                    const currentOrder = Array.isArray(answers[currentQuestion.id]) ? answers[currentQuestion.id] : 
                                        [...(currentQuestion.items || [])].map((_, idx) => idx); // Default to original order
                    
                    // Find the position of this item in the current order
                    const currentPosition = currentOrder.indexOf(originalIndex);
                    const displayPosition = currentPosition !== -1 ? currentPosition + 1 : originalIndex + 1;
                    
                    return (
                      <div
                        key={`chrono-${originalIndex}`}
                        className="border border-gray-200 rounded-lg p-3 bg-white hover:bg-gray-50 transition-colors"
                      >
                        <div className="flex items-start space-x-3">
                          <div className="flex flex-col items-center space-y-2">
                            <div className="w-8 h-8 bg-blue-100 border border-blue-300 rounded-full flex items-center justify-center text-blue-800 text-sm font-medium">
                              {displayPosition}
                            </div>
                            <div className="flex flex-col space-y-1">
                              <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={() => moveItemUp(currentQuestion.id, originalIndex)}
                              ...
                              <Button
                                ...
                                variant="outline"
                                size="sm"
                                onClick={() => moveItemDown(currentQuestion.id, originalIndex)}
                          ...
                          <div className="flex-1">
                            <span className="block text-gray-900">{itemText}</span>
                ...
                {/* Show current order */}
                {Array.isArray(answers[currentQuestion.id]) && currentQuestion.items && currentQuestion.items.length > 0 && (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-blue-800 text-sm font-medium mb-1">
                      📋 Current Order:
                    </p>
                    <p className="text-blue-700 text-sm">
                      {answers[currentQuestion.id].map((idx, pos) => {
                        const item = currentQuestion.items[idx];
                        const itemText = typeof item === 'string' ? item : (item?.text || `Item ${idx + 1}`);
                        return `${pos + 1}. ${itemText.substring(0, 30)}${itemText.length > 30 ? '...' : ''}`;
                      }).join(' → ')}
                    </p>
                  </div>
                )}
            ...
            {currentQuestion && !['true-false', 'multiple-choice', 'select-all-that-apply', 'short-answer', 'long-form-answer', 'chronological-order'].includes(currentQuestion.type) && (
